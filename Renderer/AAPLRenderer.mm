...
static const vector_float3 CameraDistanceFromCenter = (vector_float3){0.f, -200.f, -750.f};
static const vector_float3 CameraRotationAxis       = (vector_float3){0,1,0};
static const float         CameraRotationSpeed      = 0.0025f;//*8;
...
@implementation AAPLRenderer
{
    ...
    // Shadow
    CameraOrtho _cameraShadow;
    id<MTLBuffer> _shadowViewportParamsBuffers [MaxBuffersInFlight];
    id<MTLTexture> _shadowMap;
    id<MTLRenderPipelineState> _pipeline_shadowmap;
    
    // Volumetric Lighting
    id<MTLBuffer> _volumeLightingParamsBuffers[MaxBuffersInFlight];
    ...
    id<MTLTexture> _halfRes_volumeLightingResult2;
    id<MTLTexture> _halfRes_frameDepth;
    ...
    bool _enableVolumeLighting;
    bool _enableVolumeLightingBlur;
    
    float _light_altitude;
    float _light_latitude;
    float _volumeLightingStepSize;
}
...
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView *)mtkView
{
    ...
    if(self)
    {
        ...
        _enableVolumeLighting = false;
        
        _light_altitude = 15;
        _light_latitude = 0;
        _volumeLightingStepSize = 100;
        _enableVolumeLightingBlur = false;
...
{
    _volumeLightingStepSize = slider.floatValue;
}
-(void)OnEnableVolumeLightingButton:(NSButton *)button
{
    _enableVolumeLighting = [button state] == NSControlStateValueOn;
}
-(void)OnLightAltitudeSlider:(NSSlider*)slider
{
    _light_altitude = slider.floatValue;
}
-(void)OnLightLatitudeSlider:(NSSlider*)slider
{
    _light_latitude = slider.floatValue;
}
-(void)OnBlurButton:(NSButton *)button
{
    _enableVolumeLightingBlur = [button state] == NSControlStateValueOn;
}
...
-(vector_float2) generateJitterOffsetForVolumeLighting
{
    static int n_2 = 0, d_2 = 1, n_3 = 0, d_3 = 1;
    static int count = 0;
...
- (void)loadMetalWithMetalKitView:(nonnull MTKView *)mtkView
{
    ...
    for (int i = 0; i < MaxBuffersInFlight; i++)
    {
        ...
        id<MTLBuffer> shadowViewportParamsBuffer =
            [_device newBufferWithLength: sizeof(ViewportParams)
                                 options: MTLResourceStorageModeShared];
        shadowViewportParamsBuffer .label = [NSString stringWithFormat:@"shadowViewportParams[%i]", i];
        _shadowViewportParamsBuffers[i] = shadowViewportParamsBuffer;
        
        id<MTLBuffer> volumeLightingParamsBuffer =
            [_device newBufferWithLength: sizeof(VolumeLightingParams)
                                 options: MTLResourceStorageModeShared];
        shadowViewportParamsBuffer .label = [NSString stringWithFormat:@"volumeLightingParams[%i]", i];
        _volumeLightingParamsBuffers[i] = volumeLightingParamsBuffer;
    ...
    {
        id<MTLBuffer> gaussianBlurParamsBuffer =
            [_device newBufferWithLength: sizeof(GaussianBlurParams)
                                 options: MTLResourceStorageModeShared];
        gaussianBlurParamsBuffer.label = [NSString stringWithFormat:@"gaussianBlurParams_horiz"];
        _gaussianBlurParams[0] = gaussianBlurParamsBuffer;
        
        GaussianBlurParams& params = *(GaussianBlurParams*)[gaussianBlurParamsBuffer contents];
    ...
    }
    {
        id<MTLBuffer> gaussianBlurParamsBuffer =
            [_device newBufferWithLength: sizeof(GaussianBlurParams)
                                 options: MTLResourceStorageModeShared];
        gaussianBlurParamsBuffer.label = [NSString stringWithFormat:@"gaussianBlurParams_vert"];
        _gaussianBlurParams[1] = gaussianBlurParamsBuffer;
...
-(void)createTAAResource:(nonnull MTKView*)mtkView
{
    ...
    for(int i = 0;i<2;++i)
    {
        ...
        _framebuffers[i] = [_device newTextureWithDescriptor:textureDesc];
    }
    ...
    {
        MTLTextureDescriptor* textureDesc =
            [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:mtkView.colorPixelFormat width:mtkView.drawableSize.width/2 height:mtkView.drawableSize.height/2 mipmapped:NO];
        textureDesc.storageMode = MTLStorageModePrivate;
        textureDesc.usage = MTLTextureUsageRenderTarget | MTLTextureUsageShaderRead;

        _halfRes_volumeLightingResult = [_device newTextureWithDescriptor:textureDesc];
        _halfRes_volumeLightingResult2 = [_device newTextureWithDescriptor:textureDesc];
    }
    ...
    {
        MTLTextureDescriptor* textureDesc =
            [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatR32Float width:mtkView.drawableSize.width/2 height:mtkView.drawableSize.height/2 mipmapped:NO];
        textureDesc.storageMode = MTLStorageModePrivate;
        textureDesc.usage = MTLTextureUsageRenderTarget | MTLTextureUsageShaderRead;

        _halfRes_frameDepth = [_device newTextureWithDescriptor:textureDesc];
    }

    
    {
        MTLTextureDescriptor* textureDesc =
            [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatDepth32Float width:4096 height:4096 mipmapped:NO];
        textureDesc.storageMode = MTLStorageModePrivate;
        textureDesc.usage = MTLTextureUsageRenderTarget | MTLTextureUsageShaderRead;

        _shadowMap = [_device newTextureWithDescriptor:textureDesc];
    }
...
- (void)loadAssetsWithMetalKitView:(nonnull MTKView*)mtkView
{
    ...
    _actorData.lastObject.rotationPoint     = SceneCenter + (vector_float3) {-1000, -350.f, 1000.f};
    ...
    [_actorData addObject:[AAPLActorData new]];
    _actorData.lastObject.translation       = (vector_float3) {0.f, 0.f, 0.f};
    _actorData.lastObject.rotationPoint     = SceneCenter + (vector_float3) {1000.f, -350.f, 1000.f};
    _actorData.lastObject.rotationAmount    = 0.f;
    ...
    [_actorData addObject:[AAPLActorData new]];
    _actorData.lastObject.translation       = (vector_float3) {0.f, 0.f, 0.f};
    _actorData.lastObject.rotationPoint     = SceneCenter + (vector_float3) {1150.f, -350.f, -400.f};
    _actorData.lastObject.rotationAmount    = 0.f;
    ...
    [_actorData addObject:[AAPLActorData new]];
    _actorData.lastObject.translation       = (vector_float3) {0.f, 0.f, 0.f};
    _actorData.lastObject.rotationPoint     = SceneCenter + (vector_float3) {-1200.f,-350, -300.f};
    _actorData.lastObject.rotationAmount    = 0.f;
...
*/
    [_actorData addObject:[AAPLActorData new]];
    _actorData.lastObject.translation       = (vector_float3) {0.f, 0.f, 0.f};
    _actorData.lastObject.rotationPoint     = SceneCenter + (vector_float3){0.f, -355.f, 0.f};
    _actorData.lastObject.rotationAmount    = 0.f;
    ...
    _actorData.lastObject.rotationPoint     = SceneCenter + (vector_float3) {0, -350, 300};//(vector_float3) {0, -600.f, 1500.f};
    ...
    
    {
        ...
        renderDescriptor.sampleCount = 1;
        renderDescriptor.vertexFunction = [defaultLibrary newFunctionWithName:@"shadowMapVertex"];
        renderDescriptor.fragmentFunction = [defaultLibrary newFunctionWithName:@"shadowMapFragment"];
        renderDescriptor.depthAttachmentPixelFormat = MTLPixelFormatDepth32Float;
    
        _pipeline_shadowmap = [_device newRenderPipelineStateWithDescriptor:renderDescriptor error:&error];
    }
    ...
    // Volumetric Lighting
    {
        ...
        renderDescriptor.sampleCount = 1;
        renderDescriptor.vertexFunction = [defaultLibrary newFunctionWithName:@"fullscreenQuadVertex"];
        renderDescriptor.fragmentFunction = [defaultLibrary newFunctionWithName:@"volumeLightingFragment"];
        renderDescriptor.colorAttachments[0].pixelFormat = mtkView.colorPixelFormat;

        _pipeline_VolumeLighting = [_device newRenderPipelineStateWithDescriptor:renderDescriptor error:&error];
    }
    {
        MTLRenderPipelineDescriptor *renderDescriptor = [[MTLRenderPipelineDescriptor alloc] init];
        renderDescriptor.sampleCount = 1;
        renderDescriptor.vertexFunction = [defaultLibrary newFunctionWithName:@"fullscreenQuadVertex"];
        renderDescriptor.fragmentFunction = [defaultLibrary newFunctionWithName:@"depthDownsample"];
        renderDescriptor.colorAttachments[0].pixelFormat = MTLPixelFormatR32Float;

        _pipeline_DepthDownsample = [_device newRenderPipelineStateWithDescriptor:renderDescriptor error:&error];
    }
    {
        MTLRenderPipelineDescriptor *renderDescriptor = [[MTLRenderPipelineDescriptor alloc] init];
        renderDescriptor.sampleCount = 1;
    ...
    }
    {
        ...
        renderDescriptor.sampleCount = 1;
        renderDescriptor.vertexFunction = [defaultLibrary newFunctionWithName:@"fullscreenQuadVertex"];
        renderDescriptor.fragmentFunction = [defaultLibrary newFunctionWithName:@"gaussian_blur"];
        renderDescriptor.colorAttachments[0].pixelFormat = mtkView.colorPixelFormat;

        _pipeline_GaussianBlur = [_device newRenderPipelineStateWithDescriptor:renderDescriptor error:&error];
    }
...
-(vector_float3) calcLightDirection
{
    vector_float3 light_dir = {1,0,0};
    
    quaternion_float q = quaternion_from_axis_angle(vector_float3{0,0,-1}, _light_altitude*3.141592f/180.0f);
    
    light_dir = quaternion_rotate_vector(q, light_dir);
    
    q = quaternion_from_axis_angle(vector_float3{0,1,0}, _light_latitude*3.141592f/180.0f);
...
- (void)updateGameState
{
    ...
    VolumeLightingParams* volumeLightingParams = (VolumeLightingParams *)_volumeLightingParamsBuffers[_uniformBufferIndex].contents;
    ...
    // We update the shader parameters - frame constants :
    {
        ...
        const vector_float3 directionalLightDirection = [self calcLightDirection];

        _cameraShadow.target = SceneCenter;
        _cameraShadow.position = SceneCenter - directionalLightDirection * 5000;
        _cameraShadow.width = 6000;
        _cameraShadow.height = 6000;
        _cameraShadow.distanceNear = 1;
        _cameraShadow.distanceFar = 15000;
        const matrix_float4x4 shadowViewMatrix       = _cameraShadow.GetViewMatrix();
        const matrix_float4x4 shadowProjectionMatrix = _cameraShadow.GetProjectionMatrix_LH();
        ...
        frameParams[0].shadowMapViewProjMatrix = matrix_multiply(shadowProjectionMatrix, shadowViewMatrix);
        frameParams[0].frameNumber = frameNumber;
        
        ViewportParams* shadowViewportBuffer = (ViewportParams *)_shadowViewportParamsBuffers[_uniformBufferIndex].contents;
        shadowViewportBuffer->viewProjectionMatrix = frameParams[0].shadowMapViewProjMatrix ;
        
        volumeLightingParams->lightDir = directionalLightDirection;
        volumeLightingParams->cameraNearFar = {_cameraFinal.distanceNear, _cameraFinal.distanceFar};
              ...
              pass:(EPassFlags)pass
{
    ...
    viewportBuffer        =
        pass == EPassFlags::Final ? _viewportsParamsBuffers [_uniformBufferIndex] : _shadowViewportParamsBuffers[_uniformBufferIndex];
    ...
    [renderEncoder setCullMode:pass == EPassFlags::Final ? MTLCullModeBack : MTLCullModeBack];
    ...
    [renderEncoder setFragmentTexture:_shadowMap atIndex:TextureIndexShadowMap];
    ...
    for (int actorIdx = 0; actorIdx < _actorData.count; actorIdx++)
    {
        ...
        [renderEncoder setRenderPipelineState:pass == EPassFlags::Final ? lActor.gpuProg : _pipeline_shadowmap];
...
-(void)renderShadowMap : (id<MTLCommandBuffer>) cmdBuffer
{
    MTLRenderPassDescriptor* passDesc = [MTLRenderPassDescriptor renderPassDescriptor];
    passDesc.depthAttachment.clearDepth     = 1.0;
    passDesc.depthAttachment.loadAction     = MTLLoadActionClear;
    passDesc.depthAttachment.storeAction = MTLStoreActionStore;
    passDesc.depthAttachment.texture        = _shadowMap;
    
    id<MTLRenderCommandEncoder> renderEncoder =
        [cmdBuffer renderCommandEncoderWithDescriptor:passDesc];
    renderEncoder.label = @"Render ShadowMap";

    [self drawActors: renderEncoder pass: EPassFlags::ShadowMap];
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    ...
    [self renderShadowMap:commandBuffer];
    
    id<MTLTexture> currentFrameBuffer = _framebuffers[0];
    
    // Render to frame buffer
    {
        MTLRenderPassDescriptor* passDesc = [MTLRenderPassDescriptor renderPassDescriptor];
        passDesc.colorAttachments[0].clearColor = MTLClearColorMake (0.0, 0.0, 1.0, 0.0);
        ...
        passDesc.colorAttachments[0].texture    = currentFrameBuffer;
    ...
    if(_enableTAA)
    {
        // Resolve
        {
            ...
            [renderEncoder setFragmentTexture:currentFrameBuffer atIndex:0];
            [renderEncoder setFragmentTexture:_historyReset ? currentFrameBuffer : _historybuffers[0] atIndex:1];
            ...
            currentFrameBuffer = _historybuffers[1];
        }
        {
            id<MTLBlitCommandEncoder> blitEncoder = [commandBuffer blitCommandEncoder];
            
            [blitEncoder copyFromTexture:currentFrameBuffer toTexture:_historybuffers[0]];
            [blitEncoder endEncoding];
            
            currentFrameBuffer = _historybuffers[0];
        }
    }
